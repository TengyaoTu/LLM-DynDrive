...
# e.g., your custom class that supports start_new_round
...
def build_output_paths(args):
    ...
    base_name = f"{run_prefix}nowait_temp{args.temperature}_maxlen{args.max_generated_tokens}"
...
def build_bad_words_ids(tokenizer, words):
    """
    Build bad_words_ids for Transformers generate().
    We add useful variants to make suppression robust for BPETokenizers:
    - as-is, lowercase, capitalized
    - with a leading space variant
    - keep only non-empty token id sequences
    """
    variants = set()
    for w in words:
        if not w:
            continue
        base = w.strip()
        cand = {base, base.lower(), base.capitalize()}
        for c in list(cand):
            cand.add(" " + c)  # leading-space variant
        variants.update(cand)
...
def worker(rank, world_size, args):
    ...
    # Build bad_words_ids for hard suppression
    suppress_list = [w.strip() for w in args.suppress_words.split(",") if w.strip()]
    bad_words_ids = build_bad_words_ids(tokenizer, suppress_list)
    ...
    print(f"[rank {rank}] suppressing {len(suppress_list)} words -> {len(bad_words_ids)} token patterns")
    ...
    for i in my_indices:
        ...
        # Optional: safe to keep if your custom class resets internal caches between prompts
        if hasattr(model, "start_new_round"):
            ...
            with torch.no_grad():
                output = model.generate(
                    **inputs,
                    ...
                    output_scores=True,
                    bad_words_ids=bad_words_ids  # <-- hard suppression
                )
...
def main():
    ...
    parser.add_argument(
        '--suppress_words',
        type=str,
        default="wait,alternatively,hmm,but,however,alternative,another,check,double-check,oh,maybe,verify,other,again,now,ah,any",
        help="Comma-separated words to block via bad_words_ids"
    )
