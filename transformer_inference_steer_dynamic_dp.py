...
# -*- coding: utf-8 -*-
"""
Data-Parallel Dynamic Steering Inference
- Two (or more) GPUs, one process per GPU (torch.multiprocessing.spawn)
- No batching => no padding OOM
- Robust checkpointing by sample index (idx), fallback by question text
- Per-rank shard outputs; compatible with merge_shards.py I provided
- Optional --run_id to separate experiments
"""
...
# Use the user's dynamic-step patched model class
from modeling_utils.modeling_qwen2_step import Qwen2ForCausalLM
...
def enable_steering(model, tokenizer, steer_vector, args):
    if args.dynamic_steering:
        model.set_steering_flag(
            steering_flag=True,
            steering_layer=args.steer_layer,
            steer_vec=steer_vector,
            steer_coef=args.steer_coef,          # initial coef (base for dynamic)
            tokenizer=tokenizer,
            dynamic_steering=True,
            base_coef=args.steer_coef_base,
            max_coef=args.steer_coef_max,
            min_coef=args.steer_coef_min,
            update_step=True
        )
    else:
        model.set_steering_flag(
...
def worker(rank, world_size, args):
    ...
    # Steer vector: ensure on correct device
    steer_vector = torch.load(args.steer_vector_path, map_location="cpu")
    steer_vector = steer_vector.to(device)

    # Enable steering (dynamic or static)
    enable_steering(model, tokenizer, steer_vector, args)
    print(f"[rank {rank}] Steering Mode: {'Dynamic' if args.dynamic_steering else 'Static'}")
...
def main():
    ...
    # dynamic steering knobs
    parser.add_argument('--dynamic_steering', action='store_true', help='启用动态调控')
    parser.add_argument('--steer_coef_base', type=float, default=1.0)
    parser.add_argument('--steer_coef_max', type=float, default=3.0)
    parser.add_argument('--steer_coef_min', type=float, default=0.1)

    # run separation
    parser.add_argument('--run_id', type=str, default="", help='可选：标记不同实验，写入输出文件名前缀')
