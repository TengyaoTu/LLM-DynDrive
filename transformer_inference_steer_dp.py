...
def load_model_and_tokenizer(args, device):
    ...
    # ★★★ 若可用则启用更省显存注意力实现（自动fall back，不改变结果）
    try:
        if hasattr(model, "config"):
            # transformers>=4.38 的统一入口
            if hasattr(model.config, "attn_implementation"):
                model.config.attn_implementation = getattr(model.config, "attn_implementation", "sdpa")
            # PyTorch SDPA后端：优先flash->mem_efficient
